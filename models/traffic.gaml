...
species vehicle skills:[moving] {
	...
	// attributes for emission duration
	int emis_mod <- rnd(1,EMIS_DURATION);
	...
	action set_type {
		...
		// set_location for Nguyen Van Cu example
		point point_start;
		point point_end;
		float D <- road_belong.width;
		float a <- (target_node - start_node).location.x;
		float b <- (target_node - start_node).location.y;   
		point_end <- start_node + {-b*D/sqrt(a*a + b*b), a*D/sqrt(a*a + b*b)};
		if road_belong.is_twoway = true {
			point_start <- start_node;
		} else {
			point_start <- start_node*2 - point_end;
		}
		float lambda <- type = 'MOTORBIKE' ? rnd(0.0,0.3) : rnd(0.3,1.0);
		location <- (point_start*lambda + point_end*(1-lambda));
...
//	}
	...
	reflex release_emission when: mod(cycle,EMIS_DURATION) = emis_mod {
		// 20#cycle = 1#s
		// emission factor (mg/m)*|speed| (m) * 1#s
		float len <- speed * (EMIS_DURATION / 20);
		do produce_emis(EF_CO[self.type]*len, 'CO');
		do produce_emis(EF_NOx[self.type]*len, 'NOx');
		do produce_emis(EF_SO2[self.type]*len, 'SO2');
		do produce_emis(EF_PTM[self.type]*len, 'PTM');
	}
...
//			draw transfer_geom color:#red;
		...
		if (type = 'CAR') {
			draw IMAGES[0] size: {length, width} rotate:heading depth: 1#m;
		} else if (type = 'MOTORBIKE') {
			draw IMAGES[1] size: {length, width} rotate:heading depth: 1#m;
		} else if (type = 'BUS'){
			draw IMAGES[2] size: {length, width} rotate:heading depth: 1#m;
		} else if (type = 'TRUCK'){
			draw IMAGES[3] size: {length, width} rotate:heading depth: 1#m;
		}
	}
}